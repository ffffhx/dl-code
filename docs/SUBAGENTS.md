...
The main agent receives `spawn_agent`, `wait_agent`, `send_message`, `cancel_agent`, `list_agents` and `review_agent` tools. Spawn returns an ID immediately; it does not wait for the child. The third simultaneous child is rejected until a slot opens. Wait timeout (up to 60 seconds) returns the current status and does not cancel the child.
...
Each root session admits at most 32 children. `spawn_agent` accepts `acceptance_criteria` and `depends_on`. Dependencies must be existing direct children that completed and were accepted through `review_agent` with an evidence statement. Their results and reviews become explicit inputs; this does not automatically schedule a graph. Completion and acceptance are separate states. Once a result has dependents, its review and follow-up execution are frozen; create a new task for revised work. Dependency edges only point to existing tasks, preventing cycles. The parent must check evidence; a review record is not an automated proof of correctness.

## Read-only boundary and resource ownership

Children receive file reading, directory listing, bounded-depth tree inspection, ripgrep search, Skill tools, memory search/read and context-artifact reading. They receive no memory writes, shell, editor, Todo, MCP or delegation tools. This is enforced by the tool list, even if a loaded Skill asks for those capabilities. File access uses the current user's permissions; this is not an OS sandbox or a secret-file filter.
